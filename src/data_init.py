...
def get_data():
    ...
    X_train = torch.load('./datasets/raw-multivp/X_data_train.pt').to(torch.float32)
    y_train = torch.load('./datasets/raw-multivp/y_data_train.pt').to(torch.float32)
    X_test = torch.load('./datasets/raw-multivp/X_data_test.pt').to(torch.float32)
    y_test = torch.load('./datasets/raw-multivp/y_data_test.pt').to(torch.float32)
...
torch.save(y_data_old_test, 'y_data_old_test.pt')
    ...
    # load tensors from file
    X_test = torch.load('./datasets/raw-multivp/X_data_old_test.pt').to(torch.float32)
    y_test = torch.load('./datasets/raw-multivp/y_data_old_test.pt').to(torch.float32)
